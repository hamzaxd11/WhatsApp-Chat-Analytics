...
def load_data(file_name):
    ...
    data = data[(data['text_message'] != '<media omitted>') & (data['text_message'] != 'this message was deleted') & (
                data['text_message'] != 'you deleted this message')]
    data['text_message'] = data['text_message'].str.split()

    data['text_message'] = data['text_message'].apply(lambda x: ' '.join(
        item for item in x if item not in stop and 'http' not in item and not item.startswith('@') and '@' not in item))
...
def emojis_by_user(emoji_data):
    emoji_count = pd.DataFrame(
        emoji_data.groupby(['sender', 'emoji']).size().sort_values(ascending=False)).reset_index()

    emoji_count.columns = ['sender', 'emoji', 'count']

    for i in users:
        emoji_count_by_sender_top_n = emoji_count[emoji_count['sender'] == i].head(5).sort_values(by='count',
...
if __name__ == '__main__':
    ...
    menu = ["Home", "Message Count by User", "Word Usage by Person", "Plot Time Series", "Chat Activity",
            "Most Active Days", "Word Cloud", "Individual Word Cloud",
